...
```c
int epfd = epoll_create(1024);          // 1. 创建 epoll 模型

epoll_ctl(epfd, EPOLL_CTL_ADD, fd, &ev); // 2. 添加监听 fd
epfd   // epoll_create 返回的 epoll 文件描述符
op     // 操作类型：添加、修改、删除
	EPOLL_CTL_ADD  // 添加 fd 到 epoll 中
	EPOLL_CTL_MOD  // 修改 fd 关心的事件
	EPOLL_CTL_DEL  // 从 epoll 中删除 fd
fd     // 要操作的文件描述符，比如 listenfd、connfd
event  // 关心的事件

    
epoll_wait(epfd, events, maxevents, -1); // 3. 等待事件发生
