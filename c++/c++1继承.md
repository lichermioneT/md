...
public:
    ...
    // 1.对象里面存在纯虚函数，就是为了继承下去重新复写的。
    virtual void ShowInfo() const = 0;
    virtual void Work() const = 0;
    
    // 2.一般对象里面有虚函数，析构函数也写成虚函数的。
    virtual ~Person()
    {
        cout << "~Person()" << endl;
    }
...
{
public:
    // 1.继承了person。需要初始化person。在构造函数里面。
    Student(const string& name, int age, int stuId, double score)
        : Person(name, age)
        , _stuId(stuId)
        , _score(score)
    {}
    
    // 2.override可以省略virtual的。建议不要省
    // 虚函数重写的
...
public:
    // 1.继承下来的对象也需要初始化的
    Teacher(const string& name, int age, int jobId, double salary)
        : Person(name, age)
        , _jobId(jobId)
        , _salary(salary)
    {}
...
{
public:
    Assistant(const string& name, int age,
              int stuId, double score,
              int jobId, double salary,
              const string& course)
        : Person(name, age)
        , Student(name, age, stuId, score)
        , Teacher(name, age, jobId, salary)
        , _course(course)
    {}
...
int main()
{
    ...
    people.push_back(new Student("张三", 20, 1001, 89.5));
    people.push_back(new Teacher("李老师", 35, 2001, 8000));
    people.push_back(new Assistant("王五", 24, 1002, 95.0, 2002, 3000, "C++"));
